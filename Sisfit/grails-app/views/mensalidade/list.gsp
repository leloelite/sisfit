			...
			<table>
				...
				<g:each in="${mensalidadeInstanceList}" status="i" var="mensalidadeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${mensalidadeInstance.id}">${fieldValue(bean: mensalidadeInstance, field: "ano")}</g:link></td>
					
						<td>${fieldValue(bean: mensalidadeInstance, field: "mes")}</td>
