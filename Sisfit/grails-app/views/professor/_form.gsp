...
<div class="fieldcontain ${hasErrors(bean: professorInstance, field: 'modalidade', 'error')} required">
	<label for="modalidade">
		<g:message code="professor.modalidade.label" default="Modalidade" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="modalidade" name="modalidade.id" from="${sisfit.Modalidade.list()}" optionKey="id" required="" value="${professorInstance?.modalidade?.id}" class="many-to-one"/>
</div>
